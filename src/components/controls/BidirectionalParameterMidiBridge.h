...
private:
    ...
    void handleMidiCCInput(const RTEvent& event) {
        ...
        if (parameter_manager_) {
            reinterpret_cast<MockParameterManager*>(parameter_manager_)->setParameterValueRT(mapping.parameter_id, param_value);
    ...
    void handleParameterChange(const RTEvent& event) {
        ...
        if (parameter_manager_) {
            float param_value = reinterpret_cast<MockParameterManager*>(parameter_manager_)->getParameterValueRT(mapping.parameter_id);
            ...
            if (midi_handler_) {
                bool success = reinterpret_cast<MockMidiHandler*>(midi_handler_)->sendControlChangeRT(
